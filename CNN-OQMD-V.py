# -*- coding: utf-8 -*-
"""
Created on Thu Jun  4 21:14:13 2020
...
[comp1_set,energy_pa1_ave,_,_,_]=pickle.load(open('comp_volume_pa_oqmdf2.txt', 'rb')) 
...
x_all=np.array(x).reshape(-1, 1,9, 18) 
y_all = np.array(y) 
...
for k in range(5):
    ...
    pickle.dump([history.history['mean_absolute_error'],history.history['val_mean_absolute_error']],\
                open(name0+str(k)+'his2000.txt', 'wb'))
    ...
    ax.plot(history.history['mean_absolute_error'][1:])
    ax.plot(history.history['val_mean_absolute_error'][1:])
